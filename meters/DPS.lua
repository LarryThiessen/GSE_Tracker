...
local function FormatMeterValue(n)
    ...
    -- Classic per-second value otherwise prints as "0.00031617..."). The compare is pcall-guarded -- on
    -- Retail the live value is taint-"secret" and `<` throws, so we fall through to AbbreviateNumbers (which
    -- abbreviates the large secret numbers fine). string.format("%.0f", ...) is permitted on secret values.
